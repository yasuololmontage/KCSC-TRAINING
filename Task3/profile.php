<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        else
        {
            ...
            $stmt = $conn->prepare("UPDATE `task2` 
                                    SET `password`= ?,`email`= ?,`phone`= ?,`address`= ?,`sex`= ? 
                                    WHERE `username`= '$username'");
            $stmt->bind_param("sssss", $email, $phone, $password, $address, $sex);
            $stmt->execute();
    ...
    {
        $username = $_SESSION['username'];
        $stmt = $conn->prepare("DELETE FROM `task2` WHERE `username`= ?");
